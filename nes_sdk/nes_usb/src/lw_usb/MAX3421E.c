...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE write_buf[2], read_buf[2];
	write_buf[0] = reg + 2;
	write_buf[1] = val;
	read_buf[0] = 0, read_buf[1] = 0;
	if(XSpi_Transfer(&SpiInstance, write_buf, read_buf, 2) != 0){
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	BYTE write_buf[nbytes+1], read_buf[nbytes+1];
	write_buf[0] = reg + 2;
	read_buf[0] = 0;
	...
	if(XSpi_Transfer(&SpiInstance, write_buf, read_buf, nbytes + 1) != 0){
		xil_printf("MAX WRITE ERROR\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return data + nbytes;
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	write[1] = 0;
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	int error = XSpi_Transfer(&SpiInstance, write, read, 2);
	if(error != 0L){
...
void MAX3421E_reset(void) {
	...
	//Status = XGpio_Initialize(&Gpio_rst, XPAR_AXI_USB_RST_DEVICE_ID);
	...
	//Status = XGpio_Initialize(&Gpio_int, XPAR_AXI_USB_INT_DEVICE_ID);
